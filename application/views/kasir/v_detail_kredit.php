...
<body translate="no">
<div class="container">
<form class="well form-horizontal" action="<?php echo base_url().'kasir/detail_kredit/simpan_detail_kredit'?>" method="post" id="contact_form" enctype="multipart/form-data">
<fieldset>
...
<input name="dp" placeholder="Uang Muka" class="form-control" type="text" id="dp" value='0' min="0" >
<input type="hidden" id="dp2" name="dp2" class="form-control input-sm" style="text-align:right;margin-bottom:5px;" required>
</div>
</div>
</div>

<div class="form-group">
<label class="col-md-4 control-label">Lama Angsuran</label>
<div class="col-md-4 inputGroupContainer">
<div class="input-group">
...
<div class="input-group">
<span class="input-group-addon">Rp</span>
<input name="angsuran" placeholder="Sisa Angsuran" class="form-control" type="text" readonly value="<?= $total ?>" id='angsuran'>
</div>
</div>
</div>
...
<div class="input-group">
<span class="input-group-addon">Rp</span>
<input name="jml_uang" placeholder="Pembayaran Uang Muka" class="form-control" type="text" id="pembayaran" required>
</div>
</div>
</div>
...
<div class="input-group">
<span class="input-group-addon">Rp</span>
<input name="kembalian" placeholder="Kembalian Pembayaran Uang Muka" class="form-control" type="text" readonly>
</div>
</div>
</div>
...
<div  >
<a href="<?php echo base_url().'kasir/penjualan_kredit'?>" class="btn btn-danger" role="button">BATAL</a>
...
$(document).ready(function(){
  ...
  var angsuran=number-0;
  angsuran=(angsuran).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
  ...
  $('#dp').change(function(){
    ...
    // console.log(<?= $total; ?>);
    // console.log(number);
    if (parseInt(number)>=<?= $total?>){
      if (confirm('Uang muka yang sama atau melebih sisa angsuran bukan merupakan transaksi kredit. Ingin melakukan transaksi cash?')) {
        window.location.href = "<?php echo base_url().'kasir/penjualan/kredit_to_cash/'.$nofak ?>";
      } else {
          // Do nothing!
      }
      // alert('Maaf uang muka yang sama atau melebih sisa angsuran bukan merupakan transaksi kredit. Silahkan melakukan transaksi cash!');
    }else{
      var angsuran=<?= $total; ?>-number;
      var dp=number-0;
      $('input[name=dp2]').val(dp);
      angsuran=(angsuran).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
      dp=(dp).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
      $('input[name=angsuran]').val(angsuran);
      $('input[name=dp]').val(dp);
      $('input[name=jml_uang]').val('');
...
$(document).ready(function(){
  $('#pembayaran').change(function(){
    ...
    var number=document.getElementById("dp2").value;   
    var kembalian=pembayaran-number;
    // if (parseInt(pembayaran)==<?= $total?>){
    //   if (confirm('Pembayaran yang sama atau melebih sisa angsuran bukan merupakan transaksi kredit. Ingin melakukan transaksi cash?')) {
    //     window.location.href = "<?php echo base_url().'kasir/penjualan/kredit_to_cash/'.$nofak ?>";
    //   } else {
    //       // Do nothing!
    //   }
    // }else{
    if (parseInt(pembayaran) < parseInt(number)){
      // console.log(pembayaran+"<"+number+kembalian);
      alert ("Uang kurang!");
      $('input[name=jml_uang]').val('');
    }else{
      var pembayaran=pembayaran-0;
      kembalian=(kembalian).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
      pembayaran=(pembayaran).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
      $('input[name=jml_uang]').val(pembayaran);
...
$("#batal").click(function () {
    var nofak='<?= $nofak; ?>';
                $.ajax({
                url: "<?php echo base_url().'kasir/penjualan_kredit/delete/'.$nofak ?>",
                type: "get",
                success: function (response) {
                    console.log(response);
                    $("#message").html(response);
                    $('#cartmessage').show();
                    location.href='<?php echo base_url().'kasir/penjualan_kredit'?>';
